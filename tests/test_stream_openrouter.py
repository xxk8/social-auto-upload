...
Note: With PR1 the canonical home of these symbols is ``web_runner.routes.ai``:

  - ``_stream_openrouter`` (generator)
  - ``_get_all_keys_cached`` (caching wrapper; canonical name in ai.py)
  - ``_get_next_key`` (round-robin picker)
  - ``_mark_rate_limited`` (state mutator)
  - ``http_requests.post`` (the requests library reference inside ai.py)
"""
...
class TestStreamOpenRouterSuccess:
    ...
    @patch("web_runner.routes.ai._get_all_keys_cached")
    ...
    def test_200_streams_content_and_done(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        ...
        events = sse_events(_stream_openrouter("test/model", [{"role": "user", "content": "hi"}]))
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_empty_stream_returns_done_with_empty_content(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
...
class TestStreamOpenRouterRetryOn429:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_429_retries_with_next_key(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_multiple_429s_rotate_through_all_keys(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter

        mock_all_keys.return_value = _key_rows((1, "sk-****aa"), (2, "sk-****bb"), (3, "sk-****cc"))
...
class TestStreamOpenRouterKeyExhaustion:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_all_keys_429_emits_exhaustion_error(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_no_keys_available_emits_error(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
...
class TestStreamOpenRouterApiError:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_500_emits_error_with_message(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_401_emits_error_with_status(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
...
class TestStreamOpenRouterHttpException:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_connection_error_emits_error(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
...
class TestStreamOpenRouterKeyInfo:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_key_info_emitted_before_data(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_key_info_uses_first_matching_key(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
...
class TestStreamOpenRouterCustomParams:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_model_param_passed_to_api(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
        ...
        sse_events(_stream_openrouter("custom/model-id", [{"role": "user", "content": "hi"}]))
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_custom_max_tokens_and_temperature(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_default_max_tokens_and_temperature(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
        ...
        sse_events(_stream_openrouter("test/model", [{"role": "user", "content": "hi"}]))
...
class TestStreamOpenRouterConnectTimeout:
    ...
    @patch("web_runner.routes.ai.http_requests.post")
    @patch("web_runner.routes.ai._mark_rate_limited")
    def test_connect_timeout_is_tuple(self, mock_mark, mock_post, mock_next_key, mock_all_keys):
        from web_runner.routes.ai import _stream_openrouter
