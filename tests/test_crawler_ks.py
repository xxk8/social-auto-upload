"""Tests for the Kuaishou (ks) crawler — selectors + mock-based core tests.

Coverage:

  1. **Selector helpers** (pure Python — no browser, no PG):
     - ``photo_id_from_url`` — extraction from ``/short-video/<id>`` URLs.
     - ``parse_count`` — w/万/亿/numbers/empty/garbage.

  2. **Login module** (pure Python):
     - ``resolve_account_file`` — absolute path, relative name.

  3. **KuaishouCrawler mock-based** (no Playwright browser, no PG):
...
class TestSelectors:
    """``crawler.platforms.kuaishou.selectors`` pure-Python helpers."""

    # ── photo_id_from_url ───────────────────────────────────────────

    def test_photo_id_from_standard_short_video_url(self) -> None:
        from crawler.platforms.kuaishou.selectors import photo_id_from_url
        assert photo_id_from_url("/short-video/3xf4knxyzabc1234") == "3xf4knxyzabc1234"

    def test_photo_id_alphanumeric(self) -> None:
        from crawler.platforms.kuaishou.selectors import photo_id_from_url
        assert photo_id_from_url("/short-video/abcDEF123") == "abcDEF123"

    def test_photo_id_empty_href_returns_empty(self) -> None:
        from crawler.platforms.kuaishou.selectors import photo_id_from_url
        assert photo_id_from_url("") == ""
        assert photo_id_from_url(None) == ""  # type: ignore[arg-type]

    def test_photo_id_non_matching_path_returns_empty(self) -> None:
        from crawler.platforms.kuaishou.selectors import photo_id_from_url
        assert photo_id_from_url("/video/12345") == ""
        assert photo_id_from_url("/user/profile") == ""

    # ── parse_count ─────────────────────────────────────────────────

    def test_parse_count_plain_number(self) -> None:
        from crawler.platforms.kuaishou.selectors import parse_count
...
class TestLoginHelpers:
    """``crawler.platforms.kuaishou.login`` pure-Python helpers."""

    def test_resolve_account_file_with_absolute_path(
        self, tmp_path: Path
    ) -> None:
        """Absolute path + .json suffix returned as-is; parent dir created."""
        from crawler.platforms.kuaishou.login import resolve_account_file
    ...
    def test_resolve_account_file_relative_name(
        ...
        """Account name constructs ``cookies/ks_{name}.json`` under BASE_DIR."""
        from crawler.platforms.kuaishou.login import resolve_account_file

        monkeypatch.setattr("conf.BASE_DIR", str(tmp_path))
        result = resolve_account_file("test_user")
        expected = str(tmp_path / "cookies" / "ks_test_user.json")
        assert result == expected
        assert (tmp_path / "cookies").exists()


# ──────────────────────────────────────────────────────────────────────
# Section 3: KuaishouCrawler construction + guards (no browser)
# ──────────────────────────────────────────────────────────────────────


class TestKuaishouCrawlerConstruction:
    def test_platform_id_is_ks(self) -> None:
        from crawler.platforms.kuaishou.core import KuaishouCrawler
        assert KuaishouCrawler.platform_id == "ks"

    def test_constructor_sets_account_file(self) -> None:
        from crawler.platforms.kuaishou.core import KuaishouCrawler
        c = KuaishouCrawler(account_file="/tmp/fake.json", headless=True)
...
class TestValidateCookie:
    def test_no_account_file_logs_warning(
        ...
        from crawler.platforms.kuaishou.core import KuaishouCrawler
        c = KuaishouCrawler(account_file=None)
        with caplog.at_level("WARNING", logger="crawler.platforms.kuaishou.core"):
            c._validate_cookie()
    ...
    def test_invalid_cookie_raises_runtime_error(
        ...
        from crawler.platforms.kuaishou.core import KuaishouCrawler
        monkeypatch.setattr(
            "crawler.platforms.kuaishou.core.ks_cookie_check",
            AsyncMock(return_value=False),
        )
        c = KuaishouCrawler(account_file="/tmp/fake.json")
...
class TestAsyncSearchWithMockBrowser:
    ...
    Selectors use Kuaishou-specific ``KsCrawlSelectors`` constants.
    """
    ...
    def _page_with_cards() -> MagicMock:
        ...
        Selector keys match ``crawler.platforms.kuaishou.selectors.KsCrawlSelectors``.
        """
        page = MagicMock()
        page.goto = AsyncMock()
        ...
        def _card(title, author, play_count, href):
            ...
            # Kuaishou search result selectors
            link_el = MagicMock()
            link_el.get_attribute = AsyncMock(return_value=href)
            link_loc = MagicMock()
            link_loc.first = link_el
            m["a[href*='/short-video/']"] = link_loc

            m["[class*='title'], [class*='Title'], [class*='video-title']"] = _loc(title)
            m["[class*='author'], [class*='nickname'], [class*='user-name']"] = _loc(author)
            m["[class*='view-count'], [class*='play-count'], [class*='watch-count']"] = _loc(play_count)

            card.locator.side_effect = lambda sel, _m=m: _m.get(sel, MagicMock())
            return card

        cards = [
            _card("美食推荐", "美食博主", "1.2w", "/short-video/3xf4knxyzabc1234"),
            _card("旅游攻略", "旅行家小明", "5,678", "/short-video/abcDEF5678"),
            _card("摄影技巧", "摄影师王", "999+", ""),  # empty href → photo_id=""
        ]
        loc = MagicMock()
    ...
    def test_async_search_returns_parsed_rows(
        ...
        # 3 cards; card 2 has empty href so photo_id="" (empty href doesn't raise)
        assert len(rows) == 3
        assert rows[0]["post_id"] == "3xf4knxyzabc1234"
        assert rows[0]["title"] == "美食推荐"
        assert rows[0]["user"] == "美食博主"
        assert rows[0]["liked_count"] == 12000
        assert rows[1]["post_id"] == "abcDEF5678"
...
class TestAsyncDetailWithMockBrowser:
    ...
    Selector keys match ``KsCrawlSelectors`` detail-page constants.
    """
    ...
    def _detail_page(self) -> MagicMock:
        ...
        page.locator.side_effect = lambda sel: {
            ...
            "[class*='video-info-desc'], [class*='desc-text'], [class*='description'], [class*='caption']": _loc("这是一个测试视频的描述"),
            "[class*='author-info'], [class*='user-info'], [class*='creator-name'], [class*='profile-name']": _loc("测试作者"),
            "[class*='like-count'], [class*='digg-count'], [class*='action-item']": _loc("1.2w"),
        }.get(sel, _loc(""))
        return page
    ...
    def test_async_detail_returns_parsed_row(
        ...
        monkeypatch.setattr(c, "_open_browser_session", _fake_browser)
        monkeypatch.setattr(c, "_persist_content", MagicMock(return_value=1))
        row = asyncio.run(c._async_detail("3xf4knxyzabc1234"))
        assert row is not None
        assert row["post_id"] == "3xf4knxyzabc1234"
    ...
    def test_async_detail_field_extraction_failure_still_returns_row(
        ...
        monkeypatch.setattr(c, "_open_browser_session", _fake_browser)
        monkeypatch.setattr(c, "_persist_content", MagicMock(return_value=1))
        row = asyncio.run(c._async_detail("ks_video_1"))
        assert row is not None
...
class TestAsyncCommentsWithMockBrowser:
    ...
    Selector keys match ``KsCrawlSelectors`` comment-page constants.
    """
    ...
    def test_async_comments_returns_parsed_rows(
        ...
        rows = asyncio.run(c._async_comments("3xf4knxyzabc1234", max_count=10))
        assert len(rows) == 3
