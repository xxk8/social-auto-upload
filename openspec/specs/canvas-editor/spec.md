...
- **THEN** system returns `{ success: false, message: 画布数据过大（超过 N 字节），请精简后重试" }` with HTTP 413 (Payload Too Large) — NOT HTTP 400, because the size cap is a backpressure signal that reverse proxies (nginx `client_max_body_size`, Cloudflare limits) honor on 413 but not on 400. Conflating with 400 would break size-based backpressure at the edge.
- **AND** the `separators=(",", ":")` flag MUST be passed so the server's emitted JSON matches the client-side `JSON.stringify` byte count (without this flag Python's default `(", ", ": ")` would yield a larger payload than the client preflight and the two would disagree on the boundary); `N` in the error message is the runtime value of `SAU_STUDIO_CANVAS_MAX_SIZE` (env-tunable, default 10485760 = 10 MiB).
...
**DEFERRED to `studio-canvas-frontend`.** The system SHALL load canvas data only when the canvas editor is rendered. Canvas data SHALL NOT be included in the project detail query. The canvas fetch SHALL use an independent TanStack Query key so it can be invalidated independently of project detail. (Backend `/canvas` endpoint this Requirement depends on is fully implemented + tested; only the frontend `CanvasEditor` + `useQuery(['studio-canvas', projectId])` wiring is outstanding.)
...
**DEFERRED to `studio-canvas-frontend`.** The system SHALL auto-save canvas changes with a 3-second debounce after the user stops editing. The canvas editor SHALL display a save-status indicator. When there are unsaved local changes, the system SHALL warn the user before they navigate away. (Backend `PATCH /canvas` endpoint this auto-save targets is fully implemented + tested; only the editor-side `editor.store.listen()` + `beforeunload` wiring is outstanding.)
...
**DEFERRED to `studio-canvas-frontend`.** The system SHALL preflight-check the canvas payload size before sending PATCH so that the user gets immediate local feedback rather than waiting for the server's 413 (Payload Too Large) response — see Requirement 3 "Save exceeds UTF-8 byte size limit" Scenario for the backend boundary. (Backend's UTF-8 byte measurement is fully specified in Requirement 1 and verified by `test_oversized_canvas_returns_413`; only the client-side `TextEncoder().encode(JSON.stringify(...))` preflight wiring is outstanding.)
