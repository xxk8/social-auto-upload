...
// hoverTargetGroupId, handleStartRename, handleStartAuthorize,
// handleDeleteGroup. (No getPlatformLabel / handleSelectGroup in
// the SortableGroup surface.)
...
// Stub SortableAuthorizationList — the inner authorization list
// is a separate component with its own dnd-kit context. For a
// focused i18n test on SortableGroup's chrome (the action
// cluster + summary line), we render a minimal placeholder so
// the aria-label assertions don't have to navigate the inner
// list's own draggable elements.
vi.mock('./SortableAuthorizationList', () => ({
  SortableAuthorizationList: () => (
    <div data-testid="stub-sortable-auth-list" />
  ),
}))
...
function renderItem(group: AccountGroup = mockGroup) {
  return render(
    <MemoryRouter>
      <SortableGroup group={group} index={0} />
    </MemoryRouter>,
  )
}
...
describe('SortableGroup — i18n aria-labels for action buttons', () => {
  it('renders the 3 action buttons with the i18n aria-labels (rename / add / delete)', () => {
    ...
    // trips red here. These 3 keys are SHARED with GroupListItem
    // (list view) — a future rename in one file without the
