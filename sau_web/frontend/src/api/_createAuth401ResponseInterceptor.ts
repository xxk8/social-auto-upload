 ...
 *
 * Registered once on the single axios instance in `request.ts`.
 * Extracted so the redirect / race-window contract is unit-testable
 * without mounting the full client barrel.
 *
 * ## Behavior contract
 *
 ...
 *   to `navigate({ to: '/login' as never })` via React Router.
 *
 ...
 *   this file → authStore → authApi → client.ts → request.ts
 *
...
import { isLocalShellMode } from '../features/auth/localShell'
...
export function createAuth401ResponseInterceptor(
  ...
  return (error: AxiosError): Promise<never> => {
    ...
    // Local Web Shell: Flask has no multi-user JWT. Propagate 401 to
    // the call site (e.g. missing optional SaaS routes) without wiping
    // state or hard-redirecting to /login.
    if (isLocalShellMode()) {
      return Promise.reject(error)
    }
