 ...
 *
 * ## Why a shared helper (not inlined in request.ts)
 *
 * There is a **single** axios instance (`request.ts`). Header logic
 * lives here so unit tests can import and exercise it without
 * spinning up the full interceptor stack, and so future interceptors
 * stay thin.
 *
