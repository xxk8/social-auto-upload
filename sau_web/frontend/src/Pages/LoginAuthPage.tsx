...
export default function LoginAuthPage() {
  ...
  // actual `navigate()`, give the user 2s of visible "登录成功" feedback
  // (toast + button state swap) so the redirect doesn't feel abrupt on
  // fast localhost.
  //
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => navigate(`/login/forgot-password?email=${encodeURIComponent(passwordForm.watch('email') || '')}`)}
                      >
                        忘记密码？
                      </button>
                    </div>
