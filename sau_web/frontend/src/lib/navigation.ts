...
let _navigate: ((opts: { to: string; replace?: boolean }) => void) | null = null
 ...
 */
export function registerNavigate(fn: unknown): () => void {
  _navigate = fn as typeof _navigate
  return () => {
...
export function navigateInApp(
  ...
  if (_navigate) {
    ;(_navigate as any)({ to: path as never, replace: opts.replace })
    return
  }
